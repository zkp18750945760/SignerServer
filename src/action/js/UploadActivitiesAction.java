...
public class UploadActivitiesAction extends ActionSupport {
    ...
    private void insertDataToDB(InputStream inputStream) {
        try {
            ...
            String sql = "select table_name from information_schema.tables where table_name = 'activities'";
            ...
            if (tabCount == 0) {
                //数据库中还没有活动表
                sql = "create table activities(actName varchar(20) not null," +
                        "actTime varchar(20) not null," +
                        "actLocation varchar(20) not null," +
                        "primary key(actName,actTime))";
            ...
            if (values != null) {
                for (int i = 1; i < values.length; i++) {
                    tabCount = 0;
                    sql = "select * from activities where (actName,actTime) in (('" + values[i][0] + "','" + values[i][1] + "'))";
                    dbHelper = new DBHelper(sql);
                    ...
                    if (tabCount == 0) {
                        //直接插入
                        sql = "insert into activities(actName,actTime,actLocation) values('"
                                + values[i][0] + "','"
                                + values[i][1] + "','"
                                + values[i][2] + "')";
                        dbHelper = new DBHelper(sql);
                        ...
                        sql = "update activities set actLocation = '" + values[i][2] + "' " +
                                "where (actName,actTime) in (('" + values[i][0] + "','" + values[i][1] + "'))";
                        dbHelper = new DBHelper(sql);
                        dbHelper.pst.executeUpdate();
                        dbHelper.close();
                    }
